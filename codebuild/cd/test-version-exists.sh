...
AWSCRTAUTH_STATUS=$(curl -IsL https://api.nuget.org/v3/registration3/awscrt-auth/${CURRENT_TAG_VERSION}.json | grep -e ^HTTP | tail -1 | grep -c 200 || true)
...
AWSCRTAUTH_STATUS=$(curl -IsL https://api.nuget.org/v3/registration3/awscrt-auth/${CURRENT_TAG_VERSION}-rc.json | grep -e ^HTTP | tail -1 | grep -c 200 || true)
if [ $AWSCRT_STATUS -gt 0 ] && [ $AWSCRTHTTP_STATUS -gt 0 ] && [ $AWSCRTAUTH_STATUS -gt 0 ]; then
